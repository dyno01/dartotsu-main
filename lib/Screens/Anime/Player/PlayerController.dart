...
class _PlayerControllerState extends State<PlayerController> {
  ...
  Future<void> _init() async {
    ...
    // Try to restore saved audio preference for this anime
    final audioPrefKey = 'audioPreference-${media.id}';
    final savedAudioFile = loadCustomData<String>(audioPrefKey);
    bool foundPref = false;
    if (savedAudioFile != null && (currentQuality.audios?.isNotEmpty ?? false)) {
      final preferredAudio = currentQuality.audios!.firstWhereOrNull(
        (audio) => audio.file == savedAudioFile,
      );
      if (preferredAudio != null) {
        controller.setAudio(
          preferredAudio.file ?? '',
          preferredAudio.label ?? '',
          preferredAudio.file?.toNullInt() == null,
        );
        foundPref = true;
      }
    }
    // If no preference found, select the first available audio track except 'none' or 'no'
    if (!foundPref && (currentQuality.audios?.isNotEmpty ?? false)) {
      final firstAudio = currentQuality.audios!.firstWhereOrNull(
        (audio) {
          final label = (audio.label ?? '').toLowerCase();
          final file = (audio.file ?? '').toLowerCase();
          return label != 'none' && label != 'no' && file != 'none' && file != 'no';
        },
      );
      if (firstAudio != null) {
        controller.setAudio(
          firstAudio.file ?? '',
          firstAudio.label ?? '',
          firstAudio.file?.toNullInt() == null,
        );
      }
    }
      ...
      return ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(
            ...
            child: InkWell(
              onTap: () {
                // Save audio preference for this anime
                final audioPrefKey = 'audioPreference-${media.id}';
                saveCustomData<String>(audioPrefKey, sub.file ?? '');
                controller.setAudio(
                  sub.file ?? "",
                  sub.label ?? "",
                  sub.file?.toNullInt() == null,
